...
class Point:  # 使用 Point 类来接受 row 横坐标, col 纵坐标
...
class Snake:  # 定义贪吃蛇类
    def __init__(self, *, length: int, color: str, row: int, col: int, height: int = 400, width: int = 400):
    ...
    def move(self, direction_next: Direction = 'left'):  # 默认未来移动方向为向左
        head = self.head.copy()
    ...
    def hit_wall_check(self):  # 检测是否撞到墙
        dead = False
        head = self.head.copy()
        if head.col < 0 or head.row < 0 or head.row > self.playground_height or head.col > self.playground_width:
            dead = True
        return dead
